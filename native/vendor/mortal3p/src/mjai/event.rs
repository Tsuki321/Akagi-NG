...
///
/// The wire format is strict per build variant: array widths are
/// `[T; NUM_PLAYERS]` (4 for yonma, 3 for sanma), actor/kyoku bounds are
/// enforced at the serde layer, and the `nukidora` event type only exists in
/// sanma builds. Logs written for the other variant are rejected.
#[serde_as]
...
pub enum Event {
    ...
    StartKyoku {
        ...
        dora_marker: Tile,
        /// Counts from 1
        #[serde_as(deserialize_as = "TryFromInto<Kyoku>")]
        kyoku: u8,
        honba: u8,
        kyotaku: u8,
...
/// Kyoku index bound per bakaze: E1-E4/S1-S4 for yonma, E1-E3/S1-S3 for
/// sanma.
#[cfg(not(feature = "sanma"))]
type Kyoku = BoundedU8<1, 4>;
#[cfg(feature = "sanma")]
type Kyoku = BoundedU8<1, 3>;
...
impl Event {
    ...
    pub const fn actor(&self) -> Option<u8> {
        match *self {
            ...
            #[cfg(feature = "sanma")]
            Self::Nukidora { actor, .. } => Some(actor),
            _ => None,
        }
    }
    ...
    pub fn augment(&mut self) {
        ...
        match self {
            ...
            #[cfg(feature = "sanma")]
            Self::Nukidora { pai, .. } => swap_tile(pai),
...
mod test {
    ...
    fn json_consistency() {
        let lines = r#"
            {"type":"none"}
            {"type":"start_game","names":["Equim","Mortal","NoName"],"seed":[123,456]}
            {"type":"start_kyoku","bakaze":"E","dora_marker":"5s","kyoku":1,"honba":0,"kyotaku":0,"oya":0,"scores":[35000,35000,35000],"tehais":[["N","3p","W","W","7m","N","S","C","7m","P","8p","9m","5m"],["7p","1p","9m","3m","4m","C","7s","7s","9s","9p","1m","C","1s"],["3s","E","5m","P","5m","F","7p","6m","5s","9p","1s","S","N"]]}
            {"type":"tsumo","actor":0,"pai":"1m"}
            ...
            {"type":"ryukyoku","deltas":[0,1500,-1500]}
            {"type":"ryukyoku"}
            {"type":"end_kyoku"}
            {"type":"end_game"}
        "#.trim();
    ...
    fn bound_check() {
        ...
        let value = json!({
            ...
            "tehais": [
                ["N","3p","W","W","7m","N","S","C","7m","P","8p","9m","5m"],
                ["7p","1p","9m","3m","4m","C","7s","7s","9s","9p","1m","C","1s"],
                ["3s","E","5m","P","5m","F","7p","6m","5s","9p","1s","S","N"],
            ],
        });
        let obj: Map<String, Value> = json::from_value(value).unwrap();
        ...
        let mut test_obj = obj;
        test_obj["kyoku"] = Value::Number(Number::from(4));
        json::from_value::<Event>(Value::Object(test_obj)).unwrap_err();
    }
}
