...
use crate::consts::{
    ACTION_AGARI, ACTION_KAN, ACTION_PASS, ACTION_PON, ACTION_RIICHI, ACTION_RYUKYOKU,
    ACTION_SPACE, INITIAL_TILES_LEFT, MAX_VERSION, obs_shape,
};
...
impl IntegerEncoder {
    ...
    fn encode(self, ctx: &mut ObsEncoderContext<'_>) {
        ...
        match ctx.version {
            ...
            // Version 5 only exists in sanma builds; the arm is kept
            // unconditional so both variants share this encoder (v5 is
            // unreachable in yonma builds: `obs_shape(5)` panics first).
            4 | 5 => {
    ...
    fn encode_obs(mut self) -> (Array2<f32>, Array1<bool>) {
        ...
        #[cfg(not(feature = "sanma"))]
        {
            if cans.can_chi_low {
                self.arr.fill(self.idx, 1.);
                if !self.at_kan_select {
                    self.mask[ACTION_CHI_LOW] = true;
                }
            }
            if cans.can_chi_mid {
                self.arr.fill(self.idx + 1, 1.);
                if !self.at_kan_select {
                    self.mask[ACTION_CHI_MID] = true;
                }
            }
            if cans.can_chi_high {
                self.arr.fill(self.idx + 2, 1.);
                if !self.at_kan_select {
                    self.mask[ACTION_CHI_HIGH] = true;
                }
            }
        }
