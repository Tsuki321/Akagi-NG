...
use crate::consts::{MAX_VERSION, NUM_PLAYERS};
...
impl PlayerState {
    ...
    fn validate(&self) {
        ...
        if self.last_cans.can_act() {
            ...
            // Sanma: versions 1-3 are legacy 4-player encodings retained
            // only for reference; version 4 is the MahjongCopilot deployment
            // ABI exercised by the compat tooling. Only the native sanma
            // layout (MAX_VERSION) is exercised here.
            #[cfg(feature = "sanma")]
            {
                let _encoded = self.encode_obs(MAX_VERSION, false);
                if self.last_cans.can_kakan || self.last_cans.can_ankan {
                    let _encoded = self.encode_obs(MAX_VERSION, true);
                }
            }
        }
    }
...
fn rule_based_agari_all_last_minogashi() {
    ...
    #[cfg(not(feature = "sanma"))]
    let new_scores = [9000, 30000, 30000, 30000];
    #[cfg(feature = "sanma")]
    let new_scores = [9000, 30000, 30000];
    let orig_scores = mem::replace(&mut ps.scores, new_scores);
    let should_hora = ps.rule_based_agari();
    assert!(should_hora);
...
fn nukidora_allowed_during_riichi() {
    ...
    // early before the can_nukidora assignment, so a North drawn while in
    // riichi could never be extracted (kita).
    let mut ps = PlayerState::new(0);
    ...
    // must be reachable and true while a live-wall replacement remains.
    let cans = ps.test_update(&Event::Tsumo {
        actor: 0,
        pai: t!(N),
    });
    ...
    // MahjongCopilot's historical 3p ABI assigns nukidora to slot 40.
    let (_, mask) = ps.encode_obs(MAX_VERSION, false);
    assert!(mask[40], "trained mask slot 40 (nukidora) must be set");
}

#[test]
#[cfg(feature = "sanma")]
fn riichi_is_allowed_with_three_tiles_left_in_sanma() {
    let mut ps = PlayerState::new(0);
    ps.test_update(&Event::StartKyoku {
        bakaze: t!(E),
        kyoku: 1,
    ...
    // In sanma, three live-wall tiles still guarantee one more draw for the
    // declarer. The yonma threshold of four incorrectly rejects this riichi.
    ps.tiles_left = 4;
    let cans = ps.test_update(&Event::Tsumo {
        actor: 0,
        pai: t!(1m),
...
#[test]
#[cfg(feature = "sanma")]
fn pon_and_kan_use_mahjongcopilot_action_slots() {
    fn state_after_opponent_discards(player_hand: &str) -> PlayerState {
        let mut ps = PlayerState::new(0);
        ps.test_update(&Event::StartKyoku {
            bakaze: t!(E),
            kyoku: 1,
            ...
            dora_marker: t!(1p),
            tehais: [
                tile37_to_vec(&hand_with_aka(player_hand).unwrap())
                    .try_into()
                    .unwrap(),
                [t!(?); 13],
                [t!(?); 13],
    ...
    let pon_only = state_after_opponent_discards("55p 1234p 6789p 123z");
    let (_, mask) = pon_only.encode_obs(MAX_VERSION, false);
    assert!(mask[38], "pon must use action slot 38");
    assert!(
        !mask[39],
        "kan must not be legal with only two matching tiles"
    );
    assert!(!mask[40], "nukidora must not alias pon");

    let pon_or_kan = state_after_opponent_discards("555p 1234p 6789p 12z");
    let (_, mask) = pon_or_kan.encode_obs(MAX_VERSION, false);
    assert!(mask[38], "pon must use action slot 38");
    assert!(mask[39], "kan must use action slot 39");
    assert!(!mask[40], "nukidora must remain distinct from calls");
}

#[test]
#[cfg(feature = "sanma")]
fn nukidora_is_available_after_pon_before_discard() {
    let mut ps = PlayerState::new(0);
    ps.test_update(&Event::StartKyoku {
        bakaze: t!(E),
        kyoku: 1,
        ...
        dora_marker: t!(1p),
        tehais: [
            tile37_to_vec(&hand_with_aka("55s 123p 678p 346s 14z").unwrap())
                .try_into()
                .unwrap(),
            [t!(?); 13],
            [t!(?); 13],
    ...
    assert!(
        cans.can_nukidora,
        "North may be extracted after calling pon"
    );
    let (_, mask) = ps.encode_obs(MAX_VERSION, false);
    assert!(mask[40]);
}
...
#[test]
#[cfg(feature = "sanma")]
fn nukidora_does_not_share_the_four_kan_limit() {
    let mut ps = PlayerState::new(0);
    ps.test_update(&Event::StartKyoku {
        bakaze: t!(E),
        kyoku: 1,
    ...
    assert!(
        cans.can_nukidora,
        "kita replacement draws are independent from the four-kan limit"
    );
}
