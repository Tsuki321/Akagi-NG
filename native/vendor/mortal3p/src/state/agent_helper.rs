...
use crate::consts::{HONBA_TSUMO_SURCHARGE, NUM_PLAYERS};
use crate::tile::Tile;
...
impl PlayerState {
    ...
    fn rule_based_ryukyoku_slow(&self) -> bool {
        ...
        if self.is_all_last {
            ...
            let mut scores = [-3000 - self.honba as i32 * 300; NUM_PLAYERS];
    ...
    fn rule_based_agari_slow(&self, is_ron: bool, target_rel: usize) -> bool {
        ...
        if is_ron {
            ...
            exp_scores
                ...
                .skip(1)
                .for_each(|(idx, s)| {
                    // Honba surcharge per payer per tsumo: 100 in yonma
                    // (3 payers × 100 = 300 to winner), 150 in sanma
                    // (2 payers × 150 = 300 to winner).
                    let honba = self.honba as i32 * HONBA_TSUMO_SURCHARGE;
                    if idx as u8 == self.oya {
                        *s -= max_win_point.tsumo_oya + honba;
                    } else {
                        *s -= max_win_point.tsumo_ko + honba;
                    }
                });
        }
