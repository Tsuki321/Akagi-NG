...
use crate::consts::{INITIAL_TILES_LEFT, MIN_TILES_LEFT_FOR_RIICHI, NUM_PLAYERS};
...
/// Chi is only ever possible from kamicha (relative seat NUM_PLAYERS - 1)
/// and only exists in yonma. Sanma rules disable chi entirely.
///
/// NOTE: the 3p fork used to keep the yonma condition `actor_rel == 3`,
/// which is unreachable in sanma only because `rel()` collapses to {0,1,2}
/// -- an implicit range-collapse trap where the same source line means
/// "kamicha can chi" in yonma and "nobody can chi" in sanma. Keep the gate
/// explicit instead (research libriichi-divergence.md §1.4).
const CHI_ENABLED: bool = !cfg!(feature = "sanma");
...
impl PlayerState {
    ...
    fn start_kyoku(
        ...
        tehais: [[Tile; 13]; NUM_PLAYERS],
    ) -> Result<()> {
        self.tehai.fill(0);
        self.waits.fill(false);
        self.dora_factor.fill(0);
        self.tiles_seen.fill(0);
        // 2m..8m do not exist in sanma. The historical encoder includes these
        // exhausted counts in tiles_seen and the single-player calculation.
        self.tiles_seen[1..8].fill(4);
        ...
        self.jikaze = must_tile!(tu8!(E) + (NUM_PLAYERS as u8 - self.oya) % NUM_PLAYERS as u8);
        self.kyoku = kyoku - 1;
        self.is_all_last = match self.bakaze.as_u8() {
            tu8!(E) => false,
            // S4 (kyoku == 3) for yonma, S3 (kyoku == 2) for sanma.
            tu8!(S) => self.kyoku == NUM_PLAYERS as u8 - 1,
            _ => true,
        };
    ...
    fn tsumo(&mut self, actor: u8, pai: Tile) -> Result<()> {
        ...
        if self.riichi_accepted[0] {
            ...
            #[cfg(feature = "sanma")]
            {
                self.last_cans.can_nukidora = self.tehai[tuz!(N)] > 0 && self.tiles_left > 0;
            }
            return Ok(());
        }
        ...
        self.last_cans.can_riichi = self.is_menzen
            && self.tiles_left >= MIN_TILES_LEFT_FOR_RIICHI
            && self.scores[0] >= 1000
            && (self.shanten == 0 || self.shanten == 1 && self.has_next_shanten_discard);

        // Sanma: player may declare nukidora (抜きドラ) when holding a North
        // tile after a normal (non-riichi) draw. Kita replacement draws come
        // from the live wall and therefore do not share the four-kan limit.
        #[cfg(feature = "sanma")]
        {
            self.last_cans.can_nukidora = self.tehai[tuz!(N)] > 0 && self.tiles_left > 0;
        }
    ...
    fn dahai(&mut self, actor: u8, pai: Tile, tsumogiri: bool) -> Result<()> {
        ...
        // Chi is taken from kamicha, i.e. the previous seat (relative seat
        // NUM_PLAYERS - 1). Gated by CHI_ENABLED: sanma disables chi by
        // rule, explicitly (see the note at the const definition).
        if CHI_ENABLED && actor_rel == NUM_PLAYERS - 1 && !pai.is_jihai() && self.tehai_len_div3 > 0
        {
    ...
    fn pon(&mut self, actor: u8, target: u8, pai: Tile, consumed: [Tile; 2]) -> Result<()> {
        ...
        // Sanma: nukidora (抜きドラ) stays available after calling pon,
        // before the following discard.
        #[cfg(feature = "sanma")]
        {
            self.last_cans.can_nukidora = self.tehai[tuz!(N)] > 0 && self.tiles_left > 0;
        }
    ...
    pub(super) const fn rel(&self, actor: u8) -> usize {
        ((actor + NUM_PLAYERS as u8 - self.player_id) % NUM_PLAYERS as u8) as usize
    }
    ...
    /// Sanma only: declare a North tile as bonus dora (抜きドラ).
    /// The tile is removed from hand and the replacement comes from the live
    /// wall, while still counting as rinshan for yaku calculation.
    #[cfg(feature = "sanma")]
    fn nukidora(&mut self, actor: u8, pai: Tile) -> Result<()> {
        ...
        if actor_rel == 0 {
            ...
            // Keep the deployed v4 dora-owned observation: extraction does not
            // add a synthetic dora count to this field.
